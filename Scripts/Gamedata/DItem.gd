...
func get_data() -> Dictionary:
	...
	if moveable:
		data["weight"] = weight
	else: # Support shape only applies to static item
...
func on_data_changed(oldditem: DItem):
	var old_container_group = oldditem.function.container_group
	var new_container_group = function.container_group
	var old_destruction_group = oldditem.destruction.group
	var old_disassembly_group = oldditem.disassembly.group
	var changes_made = false

	# Handle container itemgroup changes
	changes_made = Gamedata.update_reference(old_container_group, new_container_group, id, "item") or changes_made

	# Handle destruction group changes
	changes_made = Gamedata.update_reference(old_destruction_group, destruction.group, id, "item") or changes_made

	# Handle disassembly group changes
	changes_made = Gamedata.update_reference(old_disassembly_group, disassembly.group, id, "item") or changes_made
...
func delete():
	...
	changes_made = Gamedata.dremove_reference(references, "core", "item", itemgroup) or changes_made
	changes_made = Gamedata.dremove_reference(references, "core", "item", destruction.group) or changes_made
	changes_made = Gamedata.dremove_reference(references, "core", "item", disassembly.group) or changes_made
	
	var mapsdata = Helper.json_helper.get_nested_data(references, "core.maps")
	Gamedata.maps.remove_entity_from_selected_maps("item", id, mapsdata)
	if changes_made:
		Gamedata.save_data_to_file(Gamedata.data.itemgroups)
	else:
...
# Removes any instance of an itemgroup from the item
